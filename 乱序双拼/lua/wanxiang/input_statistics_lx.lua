...
-- input_statistics_lx.lua：分设备统计 / 最近速度 / 峰速格式去重 / 历史查询
-- 适配乱序双拼：仅依赖公共的 wanxiang/userdb.lua，不依赖其它模块。
...
-- 内联自 wanxiang/wanxiang.lua 的输入类型识别：读取 speller/algebra 中的标记。
-- 乱序双拼等未携带标记的方案回退为方案显示名，避免显示 unknown。
-- 编号与 wanxiang_algebra_lx.yaml 中各乱序方案的标记一一对应。
...
local function get_input_method_type(env)
    ...
    if algebra then
        for i = 0, algebra.size - 1 do
            ...
            if rule then
                for _, symbol in ipairs(INPUT_METHOD_MARKER_ORDER) do
                    if rule:find(symbol, 1, true) then
                        local id = INPUT_METHOD_MARKERS[symbol]
    ...
    -- 乱序双拼等未设置标记的方案：回退到方案显示名。
    return env.engine.schema.schema_name or "unknown"
end
...
local function format_summary(title, subtitle, data, env)
    ...
    local style = get_input_method_type(env)
