...
do
    local letters = "abcdefghijklmnopqrstuvwxyz"
    local digits = "hbfdwfghijjlmnxhqnmtuvwxyw"
    for i = 1, #letters do LXSQ_MAP[s_sub(letters, i, i)] = s_sub(digits, i, i) end
end
...
local function collect_build_tasks(config, ns)
    ...
    for i = 0, list.size - 1 do
        ...
        if rule then
            ...
            value = rule:get_value("lxsq_optimization")
            local lxsq = value and value:get_bool() or false

            each_file_value(rule, function(file_value)
                local source = file_value:get_string()
                if source and source ~= "" then
                    local conversion, conversion_kind
                    if t9 then
                        conversion, conversion_kind = T9_MAP, "t9"
                    elseif lxsq then
                        conversion, conversion_kind = LXSQ_MAP, "lxsq"
                    end
                    ...
                    tasks[#tasks + 1] = {
                        ...
                        preedit_delim = (t9 or lxsq) and "==" or nil
                    }
                end
            end)
        end
...
function M.init(env)
    ...
    if rule_list then
        for i = 0, rule_list.size - 1 do
            ...
            local lxsq_val = rule:get_value("lxsq_optimization")
            local lxsq_opt = lxsq_val and lxsq_val:get_bool() or false
            local preedit_delim = (t9_opt or lxsq_opt) and "==" or nil
