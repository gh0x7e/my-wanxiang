...
万象拼音（rime-wanxiang）的乱序双拼定制分支，随上游同步 v18.0.11。
...
- `图示/乱序廿三辅助码字根表.png`：**小鹤辅**的乱序廿三修改，现在的词库就是按它来的
- `图示/乱序十七辅助码/辅助码字根表.png`：乱序十七的口径（仅存储备份，非实用）
...
- `乱序双拼/`：要覆盖的部署文件（schema / algebra / 词库 / `_lx` lua / `chaifen.txt` / `custom/` 模板）
- `图示/`：键位图 + 两张字根表；`图示/乱序十七辅助码/` 是 17 键源表备份（chaifen.txt + 字根表），不用部署

## 安装

- 下载安装官方万象拼音（rime-wanxiang）
- 把 `乱序双拼/` 里的文件覆盖进 Rime 用户目录
- 勾选方案后部署
